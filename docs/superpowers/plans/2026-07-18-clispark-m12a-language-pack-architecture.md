...
**Architecture:** A new `LanguagePack` interface (`src/languages/pack.ts`) and `RegistryChecker` interface (`src/languages/registry-checker.ts`) capture everything template/language-specific beyond what `UpdateAdapter` (from M11 Tier 3) already covers. `src/languages/packs/node-oclif.ts` bundles the existing `nodeOclifAdapter`, a new npm `RegistryChecker` implementation, and today's npm-specific scaffold/validation behavior into the one concrete pack. `scaffold.ts` and `update.ts`/`manifest.ts` take the pack (or its parts) as a required parameter — no default, no hardcoded import of the concrete pack — so the generic layer cannot silently regain npm/oclif knowledge. `wizard.ts` and `cli.ts` are the two composition-root files allowed to import the `LANGUAGE_PACKS` lookup map directly (`wizard.ts` for its own default dependency wiring, `cli.ts` to resolve the pack for scaffold/update).
...
- `src/languages/packs/node-oclif.ts` is the only *production* file allowed to import `nodeOclifAdapter` from `../../update/adapters/node-oclif` (this plan doesn't touch that file at all) — its own test file legitimately imports it too, to assert `nodeOclifPack.updateAdapter` is that exact object. `src/wizard.ts` and `src/cli.ts` are the only two production files allowed to import the `LANGUAGE_PACKS` lookup map (`src/languages/index.ts`) directly — `wizard.ts` for its own default dependency, `cli.ts` to resolve the pack for scaffold/update (their respective test files also import it, same reasoning). `src/update/manifest.ts` and `src/update/update.ts` must never import it, in production code or tests — their tests exercise the generic engine through parameters only (a real `UpdateAdapter`/`templateDir`/`language`, or the fake-adapter test's synthetic one), never by reaching for a concrete pack directly.
