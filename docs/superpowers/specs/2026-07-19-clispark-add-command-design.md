...
- `static args`, aufgebaut aus den `ParameterSpec`s:
  ...
  - `boolean` → `Args.boolean({ description, required: false })` — **immer optional, siehe "Boolean ist immer optional" unten.** Real verifiziert, obwohl das bestehende Template `boolean` bisher nur als `Flags.boolean()` zeigt (`task/list.ts`s `--done`), nie als positionales `Args.boolean()`; Recherche+Test bestätigten, dass `Args.boolean()` trotzdem eine echte, funktionierende oclif-API ist (dokumentiert in `ARCHITECTURE.md`, aber vor diesem Prototyping nie in generiertem Code exerciert). oclif selbst unterstützt hier tatsächlich `required: true` korrekt (lehnt fehlenden Wert mit `Missing 1 required arg` ab) — die Optional-Beschränkung kommt von der .NET-Seite, siehe unten.
...
- `Build()` baut die Argumente aus den `ParameterSpec`s:
  ...
  - `boolean` → immer `new Argument<bool?>(name) { Arity = ArgumentArity.ZeroOrOne }` (optionaler Fall, mirrort `TaskListCommand.cs`s `all`-Argument) — **niemals `required`, siehe "Boolean ist immer optional" unten.**

**Boolean ist immer optional (real gefundene Sprach-Asymmetrie, 2026-07-19):** Ein als `required` generierter `Argument<bool>(name)` (ohne `Arity`) wird von `System.CommandLine` bei fehlendem Wert **nicht** abgelehnt — er liefert still `false` statt eines Parse-Fehlers (getestet: `dotnet run -- confirm` ohne Wert lief mit `force=False`, Exit-Code 0 durch). oclif verhält sich hier anders: `Args.boolean({ required: true })` lehnt einen fehlenden Wert korrekt mit `Missing 1 required arg` ab. Um diese Sprach-Asymmetrie nicht ins generierte Verhalten durchsickern zu lassen (ein als "Pflicht" deklarierter Parameter, der bei .NET klammheimlich optional ist), fragt der Wizard bei Typ `boolean` **gar nicht erst** nach Pflicht/Optional — Boolean-Parameter sind für beide Sprachen immer optional (Standardwert `false`/`undefined` bei Abwesenheit), was ohnehin der üblichen CLI-Konvention für Bool-Werte entspricht.
...
4. **Parameter-Schleife:** "Weiteren Parameter hinzufügen?" (Ja/Nein-Schleife). Pro Parameter: Name (gleiche Validierung `^[a-z][a-zA-Z0-9]*$` wie beim Command-Namen, muss innerhalb des Commands eindeutig sein — wird sowohl als oclif-`args`-Objektschlüssel als auch als .NET-Variablenname verwendet) → Typ (String/Integer/Boolean/Enum) → **bei String/Integer/Enum:** Pflicht/Optional abfragen; **bei Boolean:** keine Pflicht/Optional-Frage, immer optional (siehe .NET-Abschnitt, "Boolean ist immer optional") → bei Enum zusätzlich: erlaubte Werte als kommagetrennte Texteingabe. **Reihenfolge-Regel (real gefunden beim Prototyping, siehe Node-Abschnitt oben):** sobald ein optionaler Parameter (inkl. jedes Booleans) hinzugefügt wurde, ist "Pflicht" für alle weiteren Parameter in diesem Command nicht mehr wählbar (oclif lehnt Pflicht-nach-Optional zur Laufzeit mit `Invalid argument spec` ab) — die Pflicht/Optional-Frage zeigt die Option "Pflicht" nur noch an, solange kein optionaler Parameter existiert.
